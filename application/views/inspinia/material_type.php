                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-content border_box" >
                     ...
                     <tr>
                        ...
                        <td> <?php echo $type_staff['material_name']; ?></td>
                        <td> <?php echo output($type_staff['created_at']); ?></td>
                        <td> <span class=" <?= ($type_staff['is_paid'] == 1) ? 'badge badge-success' : 'badge badge-default ' ?>"><?= ($type_staff['is_paid'] == 1) ? 'Yes' : 'No' ?></span></td>
                    ...
                    <div class="ibox-content box_inner border_box">
                        ...
                        <div class="box_content">
                            ...
                            $tbl = (isset($detail['tbl'])?$detail['tbl']:' ');
                                ...
                                {
                                    ?>
                                    <form id="material_type" method="post" action="<?php echo base_url(); ?>settings/crud/<?= $detail['tbl'] ?>/update/<?= $staff_update_data['staff_update_id'] ?>">
                                    <?php
                                }
                                else
                                {
                                    ?>
                                    <form id="material_type" method="post" action="<?php echo base_url(); ?>settings/crud/<?= $detail['tbl'] ?>/add">
                                    <?php
                                }
                            ?>
    <div class="card-body">
        <div class="form-group">
            ...
            ?>
            <input type="text" class="form-control" value="<?= (isset($staff_update_data) ? $staff_update_data['material_name'] : '') ?>" name="material_name" id="material_name" required="true" placeholder="Type">
               <label for="geo_name">Is Paid</label>
            <select class="form-control" name="is_paid">
                <option value="1" <?= (isset($staff_update_data) && $staff_update_data['is_paid'] == 1)?'selected':'' ?>>Yes</option>
                <option value="0" <?= (isset($staff_update_data) && $staff_update_data['is_paid'] == 0)?'selected':'' ?>>No</option>
            </select>
